...
begin
  ...
  ComBxDate_sec.ItemIndex:=DayOf(Now()-1);  // т.к.возвращается индекс 11, начиная с ноля

  SpinEdYear_sec.Value:=CurrentYear;
  SpinEdYear_first.Value:=2000;
  TryEncodeDate(SpinEdYear_first.Value, ComBxMoth_first.ItemIndex+1,                  // плюс единица т.к. месяц в функции может принимать значения с 1,
...
{--> Events on Change Corols elemnts of date // События на изменения контролов}
